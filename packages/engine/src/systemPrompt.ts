...
When an important line contains a nested expression, constructor/builder, chained call, or composed expression, unpack it from the inside outward: show the raw inputs first, then each intermediate value or object, then the final value, object, or state produced by the whole line. Name the object that exists after each layer rather than explaining only the outermost operation.

Show the constructed shape of the result when that helps: whether it is empty or populated, how many parts, fields, items, rows, or columns it has when that can be inferred, and which keys, indexes, identifiers, or boundaries locate its values. When structured data changes shape or meaning -- collections, maps, records, tables, indexes, queues, UI state, persisted rows, trees, graphs, or API payloads -- show a tiny before/after shape when it makes the change easier to picture. Name exactly what changed: items, keys, rows, columns, index labels, identifiers, ordering, grouping, ownership, lifecycle state, missing/empty values, boundaries crossed, and which values move versus which labels/locations move.
...
// example values: when isReady = true, the body runs; when isReady = false, startup is skipped.
