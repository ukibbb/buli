import {
  ...
  type ConversationOpenAssistantTextPart,
} from "@buli/contracts";
...
function buildCompletedLines(markdownBufferText: string): CompletedLine[] {
...
function buildOpenAssistantTextPart(markdownBufferText: string): ConversationOpenAssistantTextPart | undefined {
  ...
  if (fencedCodeBlockMatch && !/\n```\s*$/.test(markdownBufferText)) {
    return ConversationOpenFencedCodeBlockPartSchema.parse({
  ...
  return ConversationOpenMarkdownTextPartSchema.parse({
...
export function appendAssistantTextDeltaToAssistantTextMessagePartBuilder(
  ...
  const nextOpenMarkdownBufferText =
    assistantTextMessagePartBuilderState.openMarkdownBufferText + normalizedAssistantTextDeltaText;
  ...
  return {
    ...
    rawMarkdownText: assistantTextMessagePartBuilderState.rawMarkdownText + normalizedAssistantTextDeltaText,
    completedContentParts: [
      ...assistantTextMessagePartBuilderState.completedContentParts,
      ...newlyCompletedContentParts,
    ],
    openMarkdownBufferText: remainingMarkdownBufferText,
    openContentPart: buildOpenAssistantTextPart(remainingMarkdownBufferText),
  };
}
...
export function buildStreamingAssistantTextConversationMessagePart(
  ...
  return AssistantTextConversationMessagePartSchema.parse({
    ...
    completedContentParts: assistantTextMessagePartBuilderState.completedContentParts,
    ...(assistantTextMessagePartBuilderState.openContentPart
      ? { openContentPart: assistantTextMessagePartBuilderState.openContentPart }
      : {}),
  });
}
...
export function buildCompletedAssistantTextConversationMessagePart(
  ...
  const finalCompletedContentParts =
    assistantTextMessagePartBuilderState.openMarkdownBufferText.length === 0
      ? assistantTextMessagePartBuilderState.completedContentParts
      : [
          ...assistantTextMessagePartBuilderState.completedContentParts,
          ...[...parseAssistantResponseIntoContentParts(assistantTextMessagePartBuilderState.openMarkdownBufferText)],
        ];
  ...
  return AssistantTextConversationMessagePartSchema.parse({
    ...
    rawMarkdownText: assistantTextMessagePartBuilderState.rawMarkdownText,
    completedContentParts: finalCompletedContentParts,
  });
}
