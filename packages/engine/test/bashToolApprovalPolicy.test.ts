...
describe("classifyBashToolApprovalRequirement", () => {
  test.each([
    ...
    "aws s3api head-object --bucket example-bucket --key app.log",
  ...
  test.each([
    ...
    ["aws s3 rm s3://example-bucket/app.log", "cloud_state_change"],
    ["aws s3 cp local.txt s3://example-bucket/local.txt", "cloud_state_change"],
    ["aws s3 sync . s3://example-bucket", "cloud_state_change"],
    ["aws s3api get-object --bucket example-bucket --key app.log app.log", "cloud_state_change"],
    ["aws lambda invoke --function-name refresh-cache response.json", "cloud_state_change"],
    ["aws ec2 terminate-instances --instance-ids i-123", "cloud_state_change"],
    ["aws configure set region us-east-1", "cloud_state_change"],
    ["aws iam list-users", "cloud_state_change"],
