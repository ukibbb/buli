...
function parseUnquotedPromptContextReference(
  ...
  const trimmedDisplayPath = trimTrailingPromptContextPunctuation(rawDisplayPath);
...
export function trimTrailingPromptContextPunctuation(promptContextText: string): string {
  const trimmedPromptContextText = promptContextText.replace(TRAILING_PROMPT_CONTEXT_PUNCTUATION, "");
  return trimmedPromptContextText.length > 0 ? trimmedPromptContextText : promptContextText;
}
