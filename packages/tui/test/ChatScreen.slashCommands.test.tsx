...
test("ChatScreen shows and submits available skills as deterministic slash commands", async () => {
  ...
  const assistantConversationRunner: AssistantConversationRunner = {
    startConversationTurn(input) {
      conversationTurnRequests.push(input);
      return neverEmittingAssistantConversationRunner.startConversationTurn(input);
    },
  };
  const renderedChatScreen = await renderChatScreen({
    assistantConversationRunner,
  ...
  expect(slashMenuFrame).toContain("/code-review");
  expect(slashMenuFrame).toContain("Review code changes");

  await renderedChatScreen.typeText("code-review");
  const submittedSkillFrame = await renderedChatScreen.pressEnter();

  expect(submittedSkillFrame).toContain("/code-review");
  expect(conversationTurnRequests).toHaveLength(1);
  expect(conversationTurnRequests[0]).toMatchObject({
...
test("ChatScreen requests overflow compaction without auto-retrying when overflow compaction is skipped", async () => {
  ...
  expect(autoCompactionRequests).toHaveLength(1);
  expect(autoCompactionRequests[0]).toMatchObject({
    selectedModelId: "gpt-5.4",
    requestTriggerKind: "context_window_overflow",
  });
  expect(autoCompactionRequests[0]?.latestContextWindowUsage).toBeUndefined();
  expect(conversationTurnRequests.map((conversationTurnRequest) => ({
    userPromptText: conversationTurnRequest.userPromptText,
    promptSource: conversationTurnRequest.promptSource,
  }))).toEqual([
...
test("ChatScreen keeps prompt draft unchanged while command help owns keyboard input", async () => {
  const renderedChatScreen = await renderChatScreen();
  ...
  const typedBehindHelpFrame = await renderedChatScreen.typeText("hidden");
  expect(typedBehindHelpFrame).toContain("help · commands + shortcuts");
  expect(typedBehindHelpFrame).not.toContain("hidden");

  const closedFrame = await renderedChatScreen.pressEscape();
  expect(closedFrame).not.toContain("help · commands + shortcuts");
  expect(closedFrame).not.toContain("hidden");
});
