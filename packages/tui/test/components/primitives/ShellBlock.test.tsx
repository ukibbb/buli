...
describe("ShellBlock", () => {
  ...
  test("caps_visible_lines_when_maxVisibleLines_is_set", async () => {
    const { captureCharFrame, renderOnce } = await testRender(
      <ShellBlock
        maxVisibleLines={2}
        outputLines={[
          { lineKind: "prompt", lineText: "$ ls -la" },
          { lineKind: "stdout", lineText: "total 42" },
          { lineKind: "stdout", lineText: "hidden line" },
        ]}
      />,
      { width: 60, height: 8 },
    );
    ...
    expect(frame).toContain("$ ls -la");
    expect(frame).toContain("total 42");
    expect(frame).not.toContain("hidden line");
    expect(frame).toContain("showing first 2 of 3 lines");
  });
});
