...
describe("ReadToolCallCard", () => {
  test("completed_renders_header_only_with_read_range_summary", async () => {
  ...
  test("completed_uses_returned_preview_start_line_for_header_range_without_rendering_preview", async () => {
    const { captureCharFrame, renderOnce } = await testRender(
      <ReadToolCallCard
        renderState="completed"
        toolCallDetail={{
    ...
    expect(frame).not.toContain("import React");
    expect(frame).not.toContain("export function App");
  });

  test("completed_does_not_render_markdown_preview_content", async () => {
    const { captureCharFrame, renderOnce } = await testRender(
      <ReadToolCallCard
        renderState="completed"
        toolCallDetail={{
  ...
  test("completed_shows_returned_read_range", async () => {
    ...
    expect(frame).not.toContain("xxxxxxxxxxxxxxxxxxxx");
